/*****************************************************************************************
	STM32 FLASH闪存存储器读写操作
	(1)STM32 FLASH每次必须写入半字(16bit).
	(2)STM32数据存储采用小端模式(高高低低原则).
	(3)STM32互联型和大容量产品页(sector)大小为2KByte,
					中容量和小容量产品页(sector)大小为1KByte.
	(4)STM32F103ZET6主存储块地址:0x08000000-0x0807FFFF
*****************************************************************************************/
#include "stmflash.h"
#include "delay.h"
#include "usart.h"
...
//解锁STM32的FLASH  
//向FLASH->KEYR写入依次FLASH_KEY1、FLASH_KEY2
//当FLASH->CR的lock位检测到正确的解锁序列时,lock位硬件清零,实现解锁,可以对Flash进行写操作
